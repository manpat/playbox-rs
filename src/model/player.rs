...
impl Player {
	pub fn handle_input(&mut self, ctx: &mut Context<'_>, world: &model::World) {
		...
		if self.free_cam {
			...
			try_move_by(world, &mut self.position, Some(&mut self.yaw), self.height, delta);
		}
	}
}
...
fn try_move_by(world: &model::World, position: &mut WorldPosition, yaw: Option<&mut f32>, mover_height: f32, delta: Vec2) {
	...
	let current_room = &world.rooms[position.room_index];
